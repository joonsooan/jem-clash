...
2) Edit -> Preferences... -> Fast Script Reload", screen.TextStyle, GUILayout.ExpandHeight(true));
        ...
        private static readonly List<GuiSection> LeftSections = CreateLeftSections(new List<ChangeMainViewButton>
            ...
            {
                string path = GetScenePath("ExampleScene");
                if (path == null)
                {
                    int userChoice = EditorUtility.DisplayDialogComplex("Example not found",
                    ...
                    if (userChoice == 2) Window.Open("com.fastscriptreload");
                }
...
custom rewrites for methods.", screen.BoldTextStyle);
                        if (GUILayout.Button("3) Create User Defined Script Override"))
                            ScriptGenerationOverridesManager.AddScriptOverride(
                                new FileInfo(LastInspectFileHotReloadStateError.FullFileName));
...
3) Error message", MessageType.Warning);
                        ...
                        if (GUILayout.Button("4) Click here to create support-pack"))
                            try
                            {
                                ...
                                FileInfo sourceCodeCombinedFile =
                                    new(LastInspectFileHotReloadStateError.SourceCodeCombinedFilePath);
                                FileInfo originalFile = new(LastInspectFileHotReloadStateError.FullFileName);
...
It'll open override file with template already in. You can read top comments that describe how to use it."
                        ...
                        foreach (UserDefinedScriptOverride scriptOverride in ScriptGenerationOverridesManager
                                     .UserDefinedScriptOverrides)
                        {
                            EditorGUILayout.BeginHorizontal();
...
In some cases it can lock/crash editor.", MessageType.Error);
                        ...
                        using (LayoutHelper.LabelWidth(320))
                        {
                            bool valueBefore = (bool)FastScriptReloadPreference.EnableExperimentalEditorHotReloadSupport
                                .GetEditorPersistedValueOrDefault();
                            ProductPreferenceBase.RenderGuiAndPersistInput(FastScriptReloadPreference
                                .EnableExperimentalEditorHotReloadSupport);
                            bool valueAfter = (bool)FastScriptReloadPreference.EnableExperimentalEditorHotReloadSupport
                                .GetEditorPersistedValueOrDefault();
...
CustomPolling - (experimental) watches files by manual polling for changes, slowest. Make sure to narrow down watchers scope to script folders",
        ...
        protected static void CreateOpenFunctionLibraryOnRippleMethodButton()
        {
            ...
            {
                MonoScript codeComponent = AssetDatabase.LoadAssetAtPath<MonoScript>(
    ...
    public class FastScriptReloadPreference : ProductPreferenceBase
    {
        ...
        private static readonly string[] ProductKeywords = { "productivity", "tools" };

        /// <summary>
        ///     Used to know when file watchers have changed from project window contextual menu (so when to update file
        ///     watchers)
        /// </summary>
        ...
        public static readonly ToggleProjectEditorPreferenceDefinition IsPartialClassSupportEnabled = new(
        ...
        public static readonly ToggleProjectEditorPreferenceDefinition EnableCustomFileWatcher = new(
            "(Experimental) Use custom file watchers", "EnableCustomFileWatcher", false);

        public static readonly EnumProjectEditorPreferenceDefinition FileWatcherImplementationInUse = new(
        ...
        public static readonly ToggleProjectEditorPreferenceDefinition WatchOnlySpecified = new(
        ...
        public static void SetCommonMaterialsShader(ShadersMode newShaderModeValue)
        {
            ...
            string[] assets = AssetDatabase.FindAssets("t:Material Point", new[] { rootToolFolder });
    ...
    public class FastScriptReloadWelcomeScreenInitializer : WelcomeScreenInitializerBase
    {
        ...
        {
            AssetPipelineAutoRefreshMode autoRefreshMode =
                (AssetPipelineAutoRefreshMode)EditorPrefs.GetInt("kAutoRefreshMode",
            ...
            int chosenOption = EditorUtility.DisplayDialogComplex("Fast Script Reload - Warning",
        ...
        private static void DisplayMessageIfLastDetourPotentiallyCrashedEditor()
        {
            ...
            if (!SessionState.GetBool(firstInitSessionKey, false))
            {
                ...
                string lastDetour = DetourCrashHandler.RetrieveLastDetour();
...
", "Ok");
        ...
        {
            FastScriptReloadPreference.ShadersMode usedShaderMode = FastScriptReloadPreference.ShadersMode.Surface;
        ...
        {
            string userId = ProductPreferenceBase
                .CreateDefaultUserIdDefinition(FastScriptReloadWelcomeScreen.ProjectName)
